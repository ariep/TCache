...
data Cached a b
  = forall m. Executable m => Cached a (a -> m b) b Integer
  deriving (Typeable)
...
instance (Indexable a) => IResource (Cached a b) where
  key ch@(Cached a  _ _ _) = cachedKeyPrefix ++ key a   -- ++ unsafePerformIO (addrStr f )
...
cachedSTM time f a = do
  ...
  fillIt ref proto = do
    let r = unsafePerformIO $ return . fromJust =<< readResource (key proto) -- !> "fillIt"
    writeDBRef ref r
    return r
